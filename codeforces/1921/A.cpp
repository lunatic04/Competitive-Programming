...
ll dist(ll a1  , ll b1 , ll a2, ll b2){
    return (ll)(ceil((double)sqrt((a1-a2)*(a1-a2)+(b1-b2)*(b1-b2))));
}

void solve(){
     vector<vector<ll>> coor;
     FOR(i,4){
        ll x,y;
        cin>>x>>y;
        coor.pb({x,y});
     ...
     if(coor[0][0]==coor[1][0] || coor[0][1]==coor[1][1]){
        s = abs(coor[0][0] -coor[1][0]);
        s += abs(coor[0][1] -coor[1][1]);
     }
     if(coor[0][0]==coor[2][0] || coor[0][1]==coor[2][1]){
          s = abs(coor[0][0] -coor[2][0]);
          s += abs(coor[0][1] -coor[2][1]);
     }
     cout<<(ll)s*s<<"\n";
